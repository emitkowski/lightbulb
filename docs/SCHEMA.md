...
|---|---|---|
| id | uuid | PK (`HasUuids`) |
| source | varchar(50) | `reddit`, `hackernews`, `github`, `vscode`, `stackoverflow`, `producthunt`, `devto`, `twitter`, `serper_alternatives`, `serper_roadmaps`, `serper_capterra`, `indiehackers`, `g2`, `appsumo`, `chrome_webstore`, `gumroad`, `freelance`, `peopleperhour`, `guru`, `larajobs`, `indeed`, `linkedin`, `apify_actor_gaps` (Layer 21 — reuses `reddit`/`github_issues` rows too, tagged with `r/webscraping`/`apify/*` categories), `paddle_customers`, `stripe_customers` (Layer 11) |
